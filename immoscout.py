...
while True:
    ...
    old_apartements = new_apartements
    new_apartements = []
    ...
    for result in results:
        if (str(result.h5.span) == 'None'):
            new_apartement = [result.h5.string, result['href'].replace('/expose/', ''), 'https://www.immobilienscout24.de' + result['href'], 'Old']
        else:
            new_apartement = [str(result.h5).rsplit('</span>')[1].replace('</h5>', ''), result['href'].replace('/expose/', ''), 'https://www.immobilienscout24.de' + result['href'], 'Old']
